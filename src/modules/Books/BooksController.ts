...
import { BookItem, FilterType } from './BookModel';
...
export const useBookListController = () => {
  ...
  const getBookList = async () => {
    try {
      ...
      const response = await POSTMethod(
        API_ENDPOINTS.END_POINT_GET_BOOKS_HANDLER,
        body,
        headers
      );
  ...
  /* ---------------- HEIGHT CALC ---------------- */
  const calculateRowHeight = (item: BookItem) => {
  ...
  const layoutProvider = useMemo(
    () =>
      new LayoutProvider(
        index => {
          const item = dataProvider.getDataForIndex(index) as BookItem;
